...
sub hit_limit_user
{
	...
	if (open (LIMIT, "/opt/local/etc/exim/virtual/limit_$username"))
	{
	...
	{
		open (LIMIT, "/opt/local/etc/exim/virtual/limit");
		$email_limit = int(<LIMIT>);
		close(LIMIT);
	}
	...
	if ($email_limit > 0)
	{
		...
		$count = (stat("/opt/local/etc/exim/virtual/usage/$username"))[7] + 1;
...
sub smtpauth
{
	...
	else
	{
		...
		open(PASSFILE, "< /opt/local/etc/exim/virtual/domains/$domain/passwd") || return "no";
		while (<PASSFILE>)
		{
			...
			if ($test_user eq $username)
			{
				...
				open(PASSFILE_ALT, "< /opt/local/etc/exim/virtual/domains/$domain/passwd_alt") || return "no";
				while (<PASSFILE_ALT>)
				{
					($test_user,$test_pass) = split(/:/,$_);
					$test_pass =~ s/\n//g; #snip out the newline at the end
...
sub find_uid_auth_id
{
	...
	if (!$unixuser)
	{
		# we need to take $domain and get the user from /opt/local/etc/exim/virtual/domain_owners
...
sub check_limits
{
	...
	my $count = 0;
	my $email_limit = 0;
	if (open (LIMIT, "/opt/local/etc/exim/virtual/limit_$name"))
	{
		$email_limit = int(<LIMIT>);
		close(LIMIT);
	}
	...
	{
		#check this users limit
		$count = (stat("/opt/local/etc/exim/virtual/usage/$name"))[7] + 1;
		...
		if ($mid ne "")
		{
			if (! -d "/opt/local/etc/exim/virtual/usage/${name}_ids")
			{
				mkdir("/opt/local/etc/exim/virtual/usage/${name}_ids", 0770);
			}

			my $mid_char = substr($mid, 0, 1);

			if (! -d "/opt/local/etc/exim/virtual/usage/${name}_ids/$mid_char")
			{
				mkdir("/opt/local/etc/exim/virtual/usage/${name}_ids/$mid_char", 0770);
			}
			
			if (! -d "/opt/local/etc/exim/virtual/usage/${name}_ids/$mid_char/$mid")
			{
				mkdir("/opt/local/etc/exim/virtual/usage/${name}_ids/$mid_char/$mid", 0770);
			}

			my $dest_str = get_b64_string("$local_part-$domain");
			my $id_file = "/opt/local/etc/exim/virtual/usage/${name}_ids/$mid_char/$mid/$dest_str";
		...
		if (($count == $email_limit) && ($is_retry != 1))
		{
			...
			open(TQ, ">>/opt/local/etc/exim/virtual/mail_task.queue");
			...
			close(TQ);
			chmod (0660, "/opt/local/etc/exim/virtual/mail_task.queue");
		}

		if ($is_retry != 1)
		{
			open(USAGE, ">>/opt/local/etc/exim/virtual/usage/$name");
			print USAGE "1";
			close(USAGE);
			chmod (0660, "/opt/local/etc/exim/virtual/usage/$name");
		}
	}
	...
	if ( ($authenticated_id ne "") && ($is_retry != 1) )
	{
		...
		if ($domain ne "")
		{
			...
			if ($user_email_limit > 0)
			{
				...
				if (! -d "/opt/local/etc/exim/virtual/domains/$domain/usage")
				{
					mkdir("/opt/local/etc/exim/virtual/domains/$domain/usage", 0770);
				}

				if (-d "/opt/local/etc/exim/virtual/domains/$domain/usage")
				{
					open(USAGE, ">>/opt/local/etc/exim/virtual/domains/$domain/usage/$user");
					print USAGE "1";
					close(USAGE);
					chmod (0660, "/opt/local/etc/exim/virtual/domains/$domain/usage/$user");
				}
			}
		}
	}
...
sub log_bandwidth
{
	...
	open (BYTES, ">>/opt/local/etc/exim/virtual/usage/$name.bytes");
	print BYTES "$bytes=$data&path=$work_path\n";
	close(BYTES);
	chmod (0660, "/opt/local/etc/exim/virtual/usage/$name.bytes");
}
...
sub get_spam_high_score_drop
{
	...
	if (open (FILTER_CONF, "/opt/local/etc/exim/virtual/domains/$domain/filter.conf"))
	{
