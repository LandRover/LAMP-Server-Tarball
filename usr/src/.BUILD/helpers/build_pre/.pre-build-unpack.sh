...
## unpack it
[ ! -d "../${APP_NAME}-${VERSION}" ] && echo "[info] unpacking ${APP_NAME}-${VERSION}" && tar -xvf "${PACKAGE}" --strip-components=1 --one-top-level=../${APP_NAME}-${VERSION} || echo "[info] found src folder: ${APP_NAME}-${VERSION}"
