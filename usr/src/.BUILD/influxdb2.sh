...
[ -z "$(getent group ${GROUP})" ] && echo "[info] Group not found, creating.." && groupadd -r ${GROUP};
[ -z "$(getent passwd ${USER})" ] && echo "[info] User ${USER} not found, creating.." && useradd -r -g ${GROUP} -M -s /bin/false -d /home/${USER} ${USER};
...
cp ../${APP_NAME}-${VERSION}/usr/bin/influxd ${BIN_DIR}/${APP_NAME}-${VERSION}/bin/;
